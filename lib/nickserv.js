(function() {
  var EventEmitter, Nick, NickServError, notices, queue, test;
  ...
  Nick = (function() {
    ...
    function Nick(irc, options) {
      ...
      checkError = function(notices, text, wait, cb, args) {
      ...
      checkSuccess = function(notices, text, wait, cb) {
      ...
      queues = {};
      nickserv = function(cmd, args, cb, notices, args2) {
        var _ref;
        ...
        return _this.send.apply(_this, [cmd].concat(args));
      };
      ...
      this.verify = function(nick, key, cb) {
        ...
        newcb = function(err) {
          if (err) {
            if (err.type === 'unknownCommand' && (verifyCmd = 'verify register')) {
              verifyCmd = 'confirm';
              _this.verify(nick, key, cb);
              return;
            } else {
              if (err) return cb(err);
            }
        ...
        return nickserv(verifyCmd, [nick, key], newcb, notices.verifyRegistration, [nick]);
      };
